...
%%%
clear all
Filename='EFK_50A_80_clean';

disp('loading...')
load(Filename)

nmol=1;         %number of molecules in the graph. 1000-2000 atoms have highest efficiency
sz=5;         %number of graphs in the cluster
enrichfactor=50; %the factor of deuterium concentration
enrichfactor_13C=5;
crkratio=0.999;    %fraction of bonds going broken
stopat=30;      %percentage of completion
sims=450;        %number of MC simulations
rep=16;       %number of reps in the parfor loop
...
ThermalProgram=[0 0.2 0.5 0.7 1; [180 180 180 180 180]+273.5];  % A thermal program where first row is fraction to completion and second row is temeprature
